...
let config_options =
  let dump_loc =
    ...
    Arg.(
      value
      & opt (some string) None ~vopt:None
  ...
  in
  let chosen_dump_type =
    let doc = "Chose the type of intermediate file to dump (cfg | cil)" in
    let dt =
      Arg.enum
        [
          (dump_type_str CIL, CIL);
          (dump_type_str CFG, CFG);
          (dump_type_str ALL, ALL);
        ]
    in
    Arg.(
      value
      & opt (some dt) ~vopt:(Some ALL) None
      & info [ "dump-type" ] ~docv:"DUMP" ~doc)
  in
